...
public class ProductServices : ProductRepository , IProductService 
{
     ...
     public void PrintTax(double price , double tax , double priceAfterTax)
    {
     ...
     {
         return Math.Round(CalculatePriceAfterTax(price, tax) - CalculateDiscountAmount(price, discount), 2);
     }

     public void PrintFinalPrice(double price , double tax , double discount)
     {
         if (discount == 0)
         Console.WriteLine($"Price after = ${CalculateFinalPrice(price , tax ,discount).ToString("0.00")}");

         else
         {
             Console.WriteLine($"Price after = ${CalculateFinalPrice(price , tax ,discount).ToString("0.00")}"+
                               $"\nAmount Deducted ${CalculateDiscountAmount(price , discount)}");
         }
     }
}
